<?php
/**
 * HCV Engine V2 - Elementor Renderer
 *
 * Renders the analyzed source tree as native Elementor containers and
 * widgets. Responsive visual fidelity is handled by the scoped CSS generator;
 * this renderer preserves a stable per-source hook for that CSS.
 */
...
class HCV_Elementor_Renderer {
    ...
    public static function render_preview($component_tree, $normalized, $include_header_footer = false) {
        ...
        if (!$include_header_footer && $main_source_id !== '' && isset($components[$main_source_id])) {
            ...
            if (!empty($element)) $output[] = $element;
        } else {
            foreach ($roots as $root_id) {
                if (!isset($components[$root_id])) continue;
                $element = self::render_component($components, $root_id, $normalized);
                if (!empty($element)) $output[] = $element;
            }
        }
    ...
    private static function find_main_source_id($normalized) {
        foreach (($normalized['source_map'] ?? array()) as $source_id => $descriptor) {
            if (isset($descriptor['node']) && $descriptor['node'] instanceof DOMElement && strtolower($descriptor['node']->tagName) === 'main') {
                return $source_id;
            }
        }
        return '';
    ...
    private static function render_component($components, $source_id, $normalized) {
        if (!isset($components[$source_id]) || !is_array($components[$source_id])) return array();

        $component = $components[$source_id];
        $type = $component['component_type'] ?? 'group';
        $tag = strtolower($component['tag'] ?? 'div');

        if (in_array($tag, array('style', 'script', 'meta', 'link', 'title', 'noscript'), true)) return array();
        ...
        if ($type === 'button') return self::button_widget($component);
        if ($type === 'link') return self::link_widget($component);
        ...
        foreach (($component['children'] ?? array()) as $child_id) {
            ...
            if (!empty($child)) $container['elements'][] = $child;
        }

        if (empty($container['elements']) && $type === 'group' && empty($component['classes'])) return array();
        return $container;
    }

    private static function heading_widget($component) {
        $tag = strtolower($component['tag'] ?? 'h2');
        return array(
            ...
            'settings' => array_filter(array(
                ...
                'align' => self::text_alignment(self::desktop_styles($component)),
    ...
    private static function image_widget($component) {
        $node = self::node_for_component($component);
        $src = $node instanceof DOMElement ? trim($node->getAttribute('src')) : '';
    ...
    private static function container_settings($component) {
        $styles = self::desktop_styles($component);
        ...
        if (in_array(strtolower($styles['display'] ?? ''), array('flex', 'inline-flex'), true)) {
    ...
    private static function desktop_styles($component) {
        if (isset($component['styles']['desktop']) && is_array($component['styles']['desktop'])) return $component['styles']['desktop'];
        return isset($component['styles']) && is_array($component['styles']) ? $component['styles'] : array();
    }
    ...
    private static function node_text($component) {
        $node = self::node_for_component($component);
        return $node instanceof DOMElement ? trim(preg_replace('/\s+/', ' ', $node->textContent)) : '';
    }
    ...
    private static function box_setting($value, $linked) {
        ...
        $bottom = $parts[2] ?? $top;
        $left = $parts[3] ?? $right;
        return array('unit' => 'px', 'top' => self::px_value($top), 'right' => self::px_value($right), 'bottom' => self::px_value($bottom), 'left' => self::px_value($left), 'isLinked' => $linked);
    }

    private static function px_value($value) {
        return preg_match('/-?[0-9.]+/', (string) $value, $match) ? floatval($match[0]) : 0;
    }
    ...
    private static function element_id($source_id) {
        return substr(md5('hcv-v2-' . $source_id), 0, 12);
    }
